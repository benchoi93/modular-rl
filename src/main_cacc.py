...
from config import *
from marlcacc.rlenv.multiCACCworld import multiCACCworld
import wrappers
...
def train(args):
    ...
    if not model_dir.exists():
        ...
        if len(exst_run_nums) == 0:
            ...
            exp_name = 'run%i' % (max(exst_run_nums) + 1)
    ...
    # Set up environment ===========================================================
    # world = multiCACCworld(args.num_agents, args.min_num_agents, args)

    envs_train_names = []
    args.graphs = dict()
    num_agents_list = [10, 15, 20, 25]

    for num_agents in num_agents_list:
        name = f"cacc_env_{num_agents}_train"
        envs_train_names += [name]
        args.graphs[name] = utils.getGraphStructureCACC(num_agents)
    ...
    args.limb_obs_size, args.max_action, envs_train = utils.registerEnvsCACC(
        envs_train_names, num_agents_list, args.max_episode_steps, args)
    max_num_limbs = max([len(args.graphs[env_name])
                        for env_name in envs_train_names])
    # create vectorized training env
    obs_max_len = max([len(args.graphs[env_name])
                      for env_name in envs_train_names]) * args.limb_obs_size
    # envs_train = [utils.makeEnvWrapper(
    #     name, obs_max_len, args.seed) for name in envs_train_names]
    ...
    if args.max_children is None:
        args.max_children = utils.findMaxChildren(
            envs_train_names, args.graphs)
    # setup agent policy
    policy = TD3.TD3(args)
    ...
    while total_timesteps < args.max_timesteps:
        ...
        if collect_done:
            ...
            if this_training_timesteps != 0:
                ...
                for i in range(num_envs_train):
                    writer.add_scalar('episode_reward/{}'.format(
                        envs_train_names[i]), episode_reward_list[i], total_timesteps)
                    writer.add_scalar('episode_len/{}'.format(
                        envs_train_names[i]), episode_timesteps_list[i], total_timesteps)
                # print to console
        ...
        if total_timesteps < args.start_timesteps:
            ...
            for i in range(num_envs_train):
                ...
                obs = np.array(
                    obs_list[i][:args.limb_obs_size * len(args.graphs[envs_train_names[i]])])
                ...
                policy_action = np.append(policy_action, np.array(
        ...
        done_list = [done_list[i] or curr_done_list[i]
                     for i in range(num_envs_train)]
        ...
        for i in range(num_envs_train):
            ...
            writer.add_scalar('instant_reward/{}'.format(
                envs_train_names[i]), reward_list[i], total_timesteps)
            ...
            new_obs = np.array(
                new_obs_list[i][:args.limb_obs_size * num_limbs])
